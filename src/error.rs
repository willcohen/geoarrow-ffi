...
/// Run an entry point's body. A panic becomes an [`EIO`] error instead of
...
mod tests {
    ...
    fn finish_writes_code_and_message() {
        ...
        // A null sink must not crash; the code still comes back.
    ...
    /// It must also be maximal: 511 two-byte characters fill 1022 of the 1023
    /// usable bytes, and a smaller count means an over-aggressive cap.
    #[test]
    fn long_message_truncates_on_a_char_boundary() {
        ...
        assert_eq!(got.chars().count(), 511);
    }

    /// The From impls route error classes to distinct errno values; a swapped
    /// arm would misreport every failure of that class.
    #[test]
    fn maps_error_classes_to_errno_values() {
        let e: Error = RsError::IOError(std::io::Error::other("disk gone")).into();
        assert_eq!(e.code, EIO);

        let e: Error = arrow_schema::ArrowError::MemoryError("oom".to_string()).into();
        assert_eq!(e.code, ENOMEM);

        let e: Error = arrow_schema::ArrowError::ParseError("bad format".to_string()).into();
